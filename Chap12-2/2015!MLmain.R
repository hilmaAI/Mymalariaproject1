...
kenya_surveys <- dhs_surveys(countryIds = "KE", surveyType = c("MIS"))
print(kenya_surveys)

# List of files to download
data_files <- c("KEHR7AFL", "KEIR7AFL", "KEPR7AFL", "KEKR7AFL")
...
dta_files <- c("~/.cache/rdhs/datasets/KEHR7AFL.dta",
               "~/.cache/rdhs/datasets/KEIR7AFL.dta",
               "~/.cache/rdhs/datasets/KEKR7AFL.dta",
               "~/.cache/rdhs/datasets/KEPR7AFL.dta")
...
names(data_list) <- c("KEHR7AFL", "KEIR7AFL", "KEKR7AFL", "KEPR7AFL")
print(data_list)

## Define datasets
# HR Files
HRdatafile <- data_list$KEHR7AFL
# PR Files
PRdatafile <- data_list$KEPR7AFL
# IR Files
IRdatafile <- data_list$KEIR7AFL
# KR Files
KRdatafile <- data_list$KEKR7AFL
...
# Path for malaria chapter 12-2. This is also where the data is stored
chap <- "/home/ani7465/Mymalariaproject1/Chap12-2"
...
colnames(PRdatafile)
head(KRdatafile$scounty)
# Get the full name (label) of a specific column
label <- attr(KRdatafile$scounty, "label")
print(label)
colnames(HRdatafile)
...
KRdata <- KRdata %>%
  mutate(ml_antimal = case_when(
    ml_fever==1 & !(h37a==1|ml13g==1|ml13n==1|h37b==1|h37c==1|ml13d==1|ml13f==1|h37e==1|h37h==1) ~ 0,
    ml_fever==1 & (h37a==1|ml13g==1|ml13n==1|h37b==1|h37c==1|ml13d==1|ml13f==1|h37e==1|h37h==1) ~ 1,
    b5==0  ~ 99),
...
KRdata <- KRdata %>%
  mutate(ml_quin_inj = case_when(
    ml_fever==1 & ml_antimal==1 & ml13f!=1 ~ 0,
    ml_fever==1 & ml_antimal==1 & ml13f==1 ~ 1,
    b5==0  ~ 99),
...
KRdata <- KRdata %>%
  mutate(ml_artes_rec = case_when(
    ml_fever==1 & ml_antimal==1 & ml13g!=1 ~ 0,
    ml_fever==1 & ml_antimal==1 & ml13g==1 ~ 1,
    b5==0  ~ 99),
...
KRdata <- KRdata %>%
  mutate(ml_artes_inj = case_when(
    ml_fever==1 & ml_antimal==1 & ml13n!=1 ~ 0,
    ml_fever==1 & ml_antimal==1 & ml13n==1 ~ 1,
    b5==0  ~ 99),
